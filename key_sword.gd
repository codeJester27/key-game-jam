...
@export var thrust_speed: float = 0.8  # Animation speed multiplier
...
func thrust(direction: Vector2):
	...
	# Orient sword (pointing right by default)
	rotation = direction.angle() + PI/2  # +90 degrees for length-wise
	
	var tween = create_tween().set_parallel(true)
	tween.set_trans(Tween.TRANS_QUINT)
	tween.set_ease(Tween.EASE_OUT)
	
	# Forward thrust (quick)
	tween.tween_property(self, "position", 
		direction * max_thrust_distance, 
		0.1 / thrust_speed
	).from_current()
	
	# Slight scale increase for effect
	tween.tween_property(self, "scale", 
		Vector2(1.2, 1.2), 
		0.15 / thrust_speed
	).from(Vector2(1, 1))
	
	# Return movement (slower)
